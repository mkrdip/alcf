...
tf.flags.DEFINE_string("data_dir", path +"/mtfmnist_data",
                       "Path to directory containing the MNIST dataset")
tf.flags.DEFINE_string("model_dir", path + "/mtfmnist_model", "Estimator model_dir")
...
def mnist_model(image, labels, mesh):
  ...
  """
  batch_dim = mtf.Dimension("hidden", FLAGS.batch_size)
...
#                  (features, labels, mode, params))
  ...
  #layout_rules = #[("batch", "rows_blocks"), ("batch", "col_blocks")]
  ...
  tf.logging.info("features = %s labels = %s mode = %s params=%s mesh_shape=%s mesh_size=%s mesh_devices=%s layout_rules=%s, mesh_impl=%s" % (features, labels, mode, params, mesh_shape, mesh_size, mesh_devices, layout_rules, mesh_impl))
...
def run_mnist():
  ...
  train_hooks = tf.train.ProfilerHook(save_steps=1000, output_dir=path)    
